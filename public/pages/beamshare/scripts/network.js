class ServerConnection {
    ...
    _endpoint() {
        ...
        if (this._config.signalingServer) {
            wsServerDomain = this._config.signalingServer;
        } else {
            const basePath = new URL('.', window.location.href).pathname;
            wsServerDomain = location.host + (basePath.endsWith('/') ? basePath : `${basePath}/`);
        }

        const normalizedDomain = wsServerDomain.endsWith('/') ? wsServerDomain : `${wsServerDomain}/`;
        const wsUrl = new URL(`${protocol}://${normalizedDomain}server`);
