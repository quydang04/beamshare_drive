...
const evaluateRequestData = function (request) {
    return new Promise(async (resolve) => {
        ...
        const beamShareUrl = request.url;
        ...
        if (files && files.length > 0) {
            ...
            const DBOpenRequest = indexedDB.open('beamshare_store');
            DBOpenRequest.onsuccess = e => {
                ...
                for (let i = 0; i < fileObjects.length; i++) {
                    ...
                    objectStoreRequest.onsuccess = _ => {
                        if (i === fileObjects.length - 1) resolve(beamShareUrl + '?share_target=files');
                    }
                }
            }
            DBOpenRequest.onerror = _ => {
                resolve(beamShareUrl);
            }
        }
        else {
            ...
            resolve(beamShareUrl + urlArgument);
        }
    });
}
