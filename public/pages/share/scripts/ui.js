class PeersUI {
    ...
    async _onKeyDown(e) {
        ...
        // close About BeamShare page on Escape
        if (e.key === "Escape") {
...
class LanguageSelectDialog extends Dialog {
    ...
    selectLanguage(e) {
        ...
        let languageCode = target && target.value !== undefined
            ? target.value
            : null;
...
class ReceiveDialog extends Dialog {
    ...
    _parseFileData(displayName, connectionHash, files, imagesOnly, totalSize, badgeClassName) {
        ...
        this.$displayName.classList.remove("badge-room-ip");
    ...
    async _displayFiles(peerId, displayName, connectionHash, files, imagesOnly, totalSize, badgeClassName) {
        ...
        if (files.length > 1) {
            ...
            try {
                ...
                filenameDownload = `BeamShare_files_${year+month+date}_${hours+minutes}.zip`;
            } catch (e) {
                console.error(e);
...
class SendTextDialog extends Dialog {
    ...
    _onRecipient(peerId, deviceName) {
        ...
        this.$peerDisplayName.classList.remove("badge-room-ip");
...
class ReceiveTextDialog extends Dialog {
    ...
    _setDocumentTitleMessages() {
        document.title = this._receiveTextQueue.length <= 1
            ? `${ Localization.getTranslation("document-titles.message-received") } - BeamShare`
            : `${ Localization.getTranslation("document-titles.message-received-plural", null, {count: this._receiveTextQueue.length + 1}) } - BeamShare`;
    }
...
class WebShareTargetUI {
    ...
    async evaluateShareTarget(shareTargetType, title, text, url) {
        if (shareTargetType === "text") {
            ...
            let openRequest = window.indexedDB.open('beamshare_store')
